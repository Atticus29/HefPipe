...
try:
	...
	has_numpy = True
	def floatStr(f):
		if f is numpy.NaN: return 'NaN' # or 'NA'
...
base_tps.reverse()
str_func = {type(None):NoneStr, bool:BoolStr, long:LongStr, int:repr, float:floatStr, complex:ComplexStr, unicode:UniStr, str:repr, list:SeqStr, tuple:SeqStr, set:SeqStr, frozenset:SeqStr, dict:DictStr} # str will override uncode in Python 3
...
class R(object): # "del r.XXX" fails on FePy-r7 (IronPython 1.1 on .NET 2.0.50727.42) if using old-style class
	...
	SpecialLocs <- function(x) {
		...
		if (!has_numpy) {
			...
			idx <- which(is.infinite(x) & x > 0)
			if (length(idx) > 0) rlt$numpy.Inf <- idx
			idx <- which(is.infinite(x) & x < 0)
			if (length(idx) > 0) rlt['-numpy.Inf'] <- idx
			}
		return(rlt)
